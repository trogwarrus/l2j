...
public class Player extends Playable
{
  ...
  private static final String UPDATE_CHARACTER = "UPDATE characters SET level=?,maxHp=?,curHp=?,maxCp=?,curCp=?,maxMp=?,curMp=?,face=?,hairStyle=?,hairColor=?,sex=?,heading=?,x=?,y=?,z=?,exp=?,expBeforeDeath=?,sp=?,reputation=?,fame=?,raidbossPoints=?,pvpkills=?,pkkills=?,clanid=?,race=?,classid=?,deletetime=?,title=?,title_color=?,online=?,clan_privs=?,wantspeace=?,base_class=?,onlinetime=?,nobless=?,power_grade=?,subpledge=?,lvl_joined_academy=?,apprentice=?,sponsor=?,clan_join_expiry_time=?,clan_create_expiry_time=?,char_name=?,bookmarkslot=?,vitality_points=?,language=?,faction=?,pccafe_points=?,kills=?,deaths=?,rebirth_count=? WHERE charId=?";
  ...
  public void setRebirthCount(int count)
  {
    ...
    getVariables().set("rebirthCount", count);
  }
  ...
  private static Player restore(int objectId)
  {
      ...
      PreparedStatement statement = con.prepareStatement(RESTORE_CHARACTER))
    {
      ...
      {
        ResultSetMetaData metaData = rset.getMetaData(); // Получаем метаданные результата
        int columnCount = metaData.getColumnCount(); // Количество столбцов в результате
        
        if (rset.next())
        {
          StringBuilder rowLog = new StringBuilder("Row: ");
          
          // Проходим по каждому столбцу в текущей строке результата
          for (int i = 1; i <= columnCount; i++)
          {
            String columnName = metaData.getColumnName(i); // Имя столбца
            Object value = rset.getObject(i); // Значение столбца
            rowLog.append(columnName).append("=").append(value).append(", ");
          }
          
          // Логируем строку
          System.out.println(rowLog.toString()); // Вывод в консоль
          ...
          player.setRebirthCount(rset.getInt("rebirth_count"));
  ...
  private void storeCharBase()
  {
      ...
      PreparedStatement statement = con.prepareStatement(UPDATE_CHARACTER))
    {
      ...
      statement.setInt(51, getRebirthCount());
  ...
  {
    RebirthSystem.performRebirth(this);
  }
}
